...
// TestSecurityPolicySchema_IncludesPasswordCheckoutDatabase verifies that the schema includes
// password_checkout_database as an attribute. This is required for backward compatibility —
// old state files contain this attribute as null (the Terraform Plugin Framework stores all
// schema-defined attributes in state, even optional ones that were never configured).
// If removed from the schema, Terraform's plan JSON processing would fail with
// "unsupported attribute" errors.
//func TestSecurityPolicySchema_IncludesPasswordCheckoutDatabase(t *testing.T) {
//	rawStateJSON := `{
//		"id": "test-policy-id",
//		"name": "test-policy",
//		"type": "default",
...
//
//	// The current schema must be able to parse old state that includes password_checkout_database.
//	// If this fails, old state files would cause "unsupported attribute" errors.
//	currentSchema := schema.Schema{
//		Attributes: convert.SecurityPolicySchema(),
//	}
//	resourceSchemaType := currentSchema.Type().TerraformType(ctx)
//
//	val, err := tftypes.ValueFromJSON([]byte(rawStateJSON), resourceSchemaType)
//	require.NoError(t, err, "current schema must accept old state with password_checkout_database")
