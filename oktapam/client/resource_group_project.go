...
func (p ResourceGroupProject) ToResourceMap() map[string]interface{} {
...
func (c OktaPAMClient) UpdateResourceGroupProject(ctx context.Context, resourceGroupID string, id string, updates map[string]interface{}) error {
	requestURL := fmt.Sprintf("/v1/teams/%s/resource_groups/%s/projects/%s", url.PathEscape(c.Team), url.PathEscape(resourceGroupID), url.PathEscape(id))
